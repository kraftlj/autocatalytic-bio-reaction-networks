% Oscillation routine, Lewis J. Kraft, Whitesides Research Group, Harvard University.
clear all;
close all;
...
X0(AlaSEt)=0.047;%AlaSEt initial concentration in reactor
...
X0(Mal)=0.0103;%Malemide initial concentration in reactor
X0(AAm)=0.321;%Acrylamide initial concentration in reactor
...
k(Hydrolysis) = 8e-5; % AlaSEt -> ROH + EtSH
...
%Initiate constants
A(FvV)=2.5e-4;%flow rate (total of two input streams with equal flow rates)
A(inputTE)=0.047;%Initial concentration of AlaSEt in input stream
...
A(inputMal)=0.0103;%Initial concentration of Malemide in input stream
A(inputAAm)=0.321;%Initial concentration of AAm in input stream

chemsyst=@(t,X)OscillationEquationsFlow(t,X,k,A);
tspan=linspace(0,simTime,300);
[T,X]=ode45(chemsyst,tspan,X0);
Thiols=X(:,CSH)+X(:,Cmpd5)+X(:,EtSH);
Thiols=Thiols.*1000; % Convert to mM
